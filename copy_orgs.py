...
def copy_courses(driver):
    ...
    with open("copy_orgs.txt", mode="r", encoding="utf-8") as copy_list:
        copy_line = copy_list.read().splitlines()
        for copy in copy_line:
            source, destination = copy.split("\t")
            
            driver.get(my_domain+"/webapps/blackboard/execute/copy_content?navItem=copy_org_content_exists&target=yes&type=org")
