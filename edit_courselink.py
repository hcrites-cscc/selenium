...
def edit_courselink(driver):
    with open("edit_courselink.txt", mode="r", encoding="utf-8") as edit_list:
        question_line = edit_list.read().splitlines()
        for question in question_line:
            course_pk1, content_pk1, fix_contents = question.split("\t")

            driver.get(my_domain+"/webapps/blackboard/content/manageCourseLink.jsp?content_id=_"+content_pk1+"_1&course_id=_"+course_pk1+"_1")
