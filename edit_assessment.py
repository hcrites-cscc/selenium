...
def edit_assessment(driver):
    with open("edit_assessment.txt", mode="r", encoding="utf-8") as edit_list:
        question_line = edit_list.read().splitlines()
        for question in question_line:
            course_pk1, test_pk1, fix_type, fix_contents = question.split("\t")
            ...
            driver.execute_script("document.getElementById('"+fix_type+"text').value=`"+fix_contents+"`;")
            assessment_type = ""
